...
# Step 1: Regenerate compile commands
Write-Host "`n[1/4] Regenerating compile_commands.json..." -ForegroundColor Yellow

if (Test-Path "build\compile_commands.json") {
    Write-Host "  ✓ Found existing compile_commands.json" -ForegroundColor Green
} else {
...
if (Test-Path "build\compile_commands.json") {
    ...
    Write-Host "  ✓ Copied to qt-gui root directory" -ForegroundColor Green
} else {
    Write-Host "  ✗ Failed to find compile_commands.json" -ForegroundColor Red
    exit 1
}
